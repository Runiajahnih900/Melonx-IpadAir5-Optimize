...
struct SettingsViewNew: View {
    ...
    @FocusState private var isRemoteLogsKeyboardVisible: Bool
    ...
    private var debugOptionsCard: some View {
        SettingsCard {
            VStack(spacing: 4) {
                ...
                Divider()
                SettingsToggle(isOn: nativeSettingsManager.setting(forKey: "remoteLogEnabled", default: false).projectedValue, icon: "antenna.radiowaves.left.and.right", label: "Remote Live Logs", infoMessage: "Streams emulator logs to an HTTP endpoint in near real-time.")
                Divider()
                VStack(alignment: .leading, spacing: 6) {
                    Text("Remote Log Endpoint")
                        .font(.caption)
                        .foregroundColor(.secondary)

                    if #available(iOS 15.0, *) {
                        TextField("http://YOUR_PC_IP:8787/log", text: nativeSettingsManager.setting(forKey: "remoteLogEndpoint", default: "").projectedValue)
                            .font(.system(.body, design: .monospaced))
                            .textFieldStyle(.roundedBorder)
                            .textInputAutocapitalization(.never)
                            .disableAutocorrection(true)
                            .focused($isRemoteLogsKeyboardVisible)
                            .toolbar {
                                ToolbarItem(placement: .keyboard) {
                                    Button("Dismiss") {
                                        isRemoteLogsKeyboardVisible = false
                                    }
                                }
                            }
                    } else {
                    ...
                    Text("Example: http://192.168.1.20:8787/log")
                        .font(.caption2)
                        .foregroundColor(.secondary)
                }
                .padding(.vertical, 4)
